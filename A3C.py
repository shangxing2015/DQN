...
class ContinuousDeepQ(object):
    ...
    def update_target_network(source_network, target_network, update_rate):
        ...
        for v_source, v_target in zip(source_network.variables(), target_network.variables()):
            ...
            update_op = v_target.assign((1 - update_rate) * v_target + update_rate * v_source)

            target_network_update.append(update_op)
        return tf.group(*target_network_update)  # why return a pointer?
    ...
    def create_variables(self):
        ...
        with tf.name_scope("taking_action"):
            ...
            self.actor_action = tf.identity(self.actor(self.observation), name="actor_action")  # identity??
        ...
        with tf.name_scope("actor_update"):
            ...
            actor_gradients = self.optimizer.compute_gradients(tf.reduce_mean(-self.actor_score),
        ...
        with tf.name_scope("target_network_update"):
            self.target_actor_update = ContinuousDeepQ.update_target_network(self.actor, self.target_actor,
                                                                             self.target_actor_update_rate)
