...
from config_2 import N_CHANNELS, DISCOUNT, N_SENSING
    ...
    def _whittle_index_discounted(self):
        ...
        for i in range(N_CHANNELS):
            ...
            if p[1][1] >= p[0][1]:
                ...
                c_2_num = math.pow(self.gamma,
                                   self._L_pos_func(p[0][1], self.belief[i], i)) * self._k_step_belief_update(
                    self._L_pos_func(p[0][1], self.belief[i], i), p[0][1], i)
                ...
                c_3_denum = 1 + (1 + self.gamma) * self.gamma * p[0][1] - math.pow(self.gamma,
                                                                                   2) * self._k_step_belief_update(1,
                                                                                                                   p[1][
                                                                                                                       1],
                                                                                                                   i)
                c_3 = c_3_num / c_3_denum
                c_4_num = self.gamma * self._k_step_belief_update(1, p[1][1], i) * (1 - self.gamma) + math.pow(
                    self.gamma, 2) * p[0][1]
                c_4_denum = 1 + (1 + self.gamma) * self.gamma * p[0][1] - math.pow(self.gamma,
                                                                                   2) * self._k_step_belief_update(1,
                                                                                                                   p[1][
                                                                                                                       1],
                                                                                                                   i)
            ...
            if p[1][1] >= p[0][1]:
                ...
                if self.belief[i] <= p[0][1] or self.belief[i] >= p[1][1]:
                    ...
                    temp_denum = 1 - p[1][1] + (self.belief[i] - self._k_step_belief_update(1, self.belief[i],
                                                                                            i)) * self._L_pos_func(
                        p[0][1], self.belief[i], i) + self._k_step_belief_update(
                        self._L_pos_func(p[0][1], self.belief[i], i), p[0][1], i)
                    self.whittle_idx[i] = temp_num * self.B[i] / temp_denum
                else:
                ...
                if self.belief[i] <= p[1][1] or self.belief[i] >= p[0][1]:
                    ...
                    temp_denum = 1 + p[0][1] - self._k_step_belief_update(1, p[1][1], i) + self._k_step_belief_update(1,
                                                                                                                      self.belief[
                                                                                                                          i],
                                                                                                                      i) - \
                                 self.belief[i]
