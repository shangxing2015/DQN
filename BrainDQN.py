...
class BrainDQN:
    ...
    def createQNetwork(self):
        ...
        W_1 = tf.Variable(
            tf.truncated_normal([int(STATE_SIZE), int(HIDDEN_UNINITS)], stddev=1.0 / math.sqrt(float(STATE_SIZE))))
        b_1 = tf.Variable(tf.zeros([HIDDEN_UNINITS]))

        hidden1 = tf.nn.tanh(tf.matmul(self.state_placeholder, W_1) + b_1)

        W_2 = tf.Variable(
            tf.truncated_normal([int(HIDDEN_UNINITS), int(ACTION_SIZE)], stddev=1.0 / math.sqrt(float(HIDDEN_UNINITS))))
    ...
    def trainQNetwork(self):
        ...
        y_batch = []
        q_value_batch = self.q_values.eval(
            feed_dict={self.state_placeholder: next_state_batch})  # data structure: [[1, 2, 4], [3,4,5]]
